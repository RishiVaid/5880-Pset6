...
"""
    _world(model::MyRectangularGridWorldModel, s::Int, a::Int)::Tuple{Int64, Float64}

Given a state `s` and an action `a`, return the next state `s′` and the reward `r`.

### Arguments
- `model::MyRectangularGridWorldModel`: The world model
- `s::Int`: The current state
- `a::Int`: The action

### Returns
- `Tuple{Int64, Float64}`: The next state and the reward
"""
...
- `data::NamedTuple`: The data tuple

The data tuple should contain the following fields:
- `s::Int`: The current state
- `a::Int`: The action taken
- `r::Float64`: The reward received
- `s′::Int`: The next state

### Returns
- Returns updated QLearning agent model
"""
...
"""
    _update!(model::MySARSAAgentModel, data::NamedTuple) -> MySARSAAgentModel
...
- `s::Int`: The current state
- `a::Int`: The action taken
- `a′::Int`: The next action
- `r::Float64`: The reward received
- `s′::Int`: The next state

### Returns
- Returns updated SARSA agent model
"""
function _update(model::MySARSAAgentModel, data::NamedTuple)::MySARSAAgentModel
    ...
    #Using the update rule to update Q_array
    Q[s,a] += α*(r+γ*maximum(Q[s′,:]) - Q[s,a])
...
function simulate(agent::MySARSAAgentModel, environment::MyRectangularGridWorldModel, 
    ...
    for a in 1:number_of_actions
        b = Array{Float64,1}(undef,number_of_actions)
        for i in 1:number_of_actions
            if (a==i)
                b[i] = 1 - ϵ + ϵ/number_of_actions;
            else
                b[i] = ϵ/number_of_actions;
            end
        end
        exploration_dictionary[a] = Categorical(b);
    end

    #simulation loop
    s = environment.states[startstate];
    a = agent.my_π[s]; # getting action from policy at this particular episode
    ...
    for _ ∈ 1:maxsteps
        ...
        a′ = agent.my_π[s′] |> i -> exploration_dictionary[i] |> rand # best possible guess to take

        #update the model

        agent = agent((s = s, a = a, r = r,s′ = s′, a′ = a′))
